...
fitCosine = function(empiricalData,
                     periodInSeconds,
                     optimizeRange,
                     nmin,
  ...
  grandMean = 10.5 #mean(means)
  ampEst = sapply(empiricalData, function(x) (max(x) - min(x))/2)
  secs = as.numeric(zoo::index(empiricalData))

  minEdRange = min(sapply(empiricalData, function(x) diff(range(x))))
  tinyBitOfNoise = runif(nrow(as.data.frame(empiricalData)),
                         min = -(minEdRange*0.000001),
                         max = minEdRange*0.000001)
  ...
  fits = mapply(
    function(eD, ampEst, obsTime, grandMean, period){
      if(length(na.omit(eD)) >= nmin) {
        ...
        #eDRange = diff(range(eD))
        #eD = eD + runif(length(eD), min = -(eDRange * 0.0001), max = eDRange * 0.0001)
        eD = eD + tinyBitOfNoise
        fitResult = nls(
